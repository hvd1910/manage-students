...
public class GiangVienDiemController extends HttpServlet {
    ...
    public GiangVienDiemController() {
        super();
    }
        ...
        if ("delete".equals(action)) {
            ...
            if (maBHToDelete != null && !maBHToDelete.isEmpty()) {
                try {
                    ...
                    response.sendRedirect(request.getContextPath() + "/giangvien/diem?msg=OK");
                } catch (Exception e) {
                    e.printStackTrace(); // Log the exception
                    response.sendRedirect(request.getContextPath() + "/giangvien/diem?msg=FAIL");
                }
            } else {
                response.sendRedirect(request.getContextPath() + "/giangvien/diem?msg=FAIL");
            }
            ...
            if ("add".equalsIgnoreCase(action)) {
                ...
                RequestDispatcher rd = request.getRequestDispatcher("/views/giangvien/diem.jsp");
                rd.forward(request, response);
            } else {
                // Forward to list page (listdiem.jsp)
                RequestDispatcher rd = request.getRequestDispatcher("/views/giangvien/listdiem.jsp");
                rd.forward(request, response);
            }
        }
        // Data list diem
            ...
            try {
                ...
                if (diemUpdate != null) {
                    ...
                    if (update > 0) {
                        ...
                        response.sendRedirect(request.getContextPath() + "/giangvien/diem?msg=ERROR");
                    }
                }else {
                    response.sendRedirect(request.getContextPath() + "/giangvien/diem?msg=ERROR");
                }
